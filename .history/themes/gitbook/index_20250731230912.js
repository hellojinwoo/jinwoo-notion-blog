...
function getNavPagesWithLatest(allNavPages, latestPosts, post) {
  // localStorage에 id와 마지막 읽은 시간 타임스탬프 저장: posts_read_time = {"${post.id}":"Date()"}
  ...
  return allNavPages?.map(item => {
    ...
    // 최신 게시글(보통 6개)에 속하며 && (읽은 기록이 없거나 || 최근 업데이트 시간이 마지막 읽은 시간보다 크면)
    if (
...
/**
 * 기본 레이아웃
 * 좌우 양측 레이아웃을 사용하고, 모바일에서는 상단 네비게이션 바를 사용
 * @returns {JSX.Element}
 * @constructor
 */
...
const LayoutIndex = props => {
  ...
  const [hasRedirected, setHasRedirected] = useState(false) // 리다이렉트 여부를 추적하는 상태 추가

  useEffect(() => {
    const tryRedirect = async () => {
      if (!hasRedirected) {
        // 리다이렉트되지 않았을 때만 실행
        setHasRedirected(true) // 상태 업데이트로 중복 실행 방지

        // 지정된 게시글로 리다이렉트
        await router.push(index)

        // setTimeout을 사용하여 페이지 로드 상태 확인
        setTimeout(() => {
          const article = document.querySelector(
            '#article-wrapper #notion-article'
          )
          if (!article) {
            console.log('Notion 데이터베이스에 이 slug 페이지가 포함되어 있는지 확인해주세요: ', index)

            // 오류 정보 표시
            const containerInner = document.querySelector(
              '#theme-gitbook #container-inner'
            )
            const newHTML = `<h1 class="text-3xl pt-12 dark:text-gray-300">설정 오류</h1><blockquote class="notion-quote notion-block-ce76391f3f2842d386468ff1eb705b92"><div>Notion에 slug가 ${index}인 게시글을 추가해주세요</div></blockquote>`
  ...
  }, [index, hasRedirected]) // hasRedirected를 의존성으로 하여 상태 변경 시 업데이트 보장

  return null // 어떤 콘텐츠도 렌더링하지 않음
}
...
const Layout404 = props => {
  ...
  useEffect(() => {
    // 3초 후에도 로드에 실패하면 홈페이지로 이동
    setTimeout(() => {
      const article = isBrowser && document.getElementById('article-wrapper')
      if (!article) {
        router.push('/').then(() => {
          // console.log('페이지를 찾을 수 없음', router.asPath)
        })
      }
    }, 3000)
  }, [])
