...
function getNavPagesWithLatest(allNavPages, latestPosts, post) {
  // localStorage 저장 id와 마지막 읽은 시간 타임스탬프: posts_read_time = {"${post.id}":"Date()"}
...
const LayoutIndex = props => {
  ...
  const [hasRedirected, setHasRedirected] = useState(false) // 리다이렉트 완료 여부를 추적하는 상태 추가
  ...
  useEffect(() => {
    const tryRedirect = async () => {
      if (!hasRedirected) {
        ...
        // setTimeout을 사용하여 페이지 로딩 상황 확인
        setTimeout(() => {
          const article = document.querySelector(
            '#article-wrapper #notion-article'
          )
          if (!article) {
            console.log('Notion 데이터베이스에 이 slug 페이지가 포함되어 있는지 확인하세요: ', index)

            // 오류 정보 표시
            const containerInner = document.querySelector(
              '#theme-gitbook #container-inner'
            )
            const newHTML = `<h1 class="text-3xl pt-12 dark:text-gray-300">설정 오류</h1><blockquote class="notion-quote notion-block-ce76391f3f2842d386468ff1eb705b92"><div>Notion에 slug가 ${index}인 게시물을 추가해주세요</div></blockquote>`
